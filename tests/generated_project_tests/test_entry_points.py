...
def test_parameter_sets_entry_points():
    ...
    entry_points = list(pybamm_reservoir_example.parameter_sets).sort()
    parameter_sets = Path("src/pybamm_reservoir_example/parameters/input/").glob("*.py")
...
def test_parameter_sets_entry_point_load():
    ...
    parameters = pybamm_reservoir_example.parameter_sets['Chen2020']
    # Loading parameter sets through the source file by dynamically loading Chen2020.py as a module
    spec = importlib.util.spec_from_file_location("Chen2020mod", "src/pybamm_reservoir_example/parameters/input/Chen2020.py")
...
def test_model_entry_points():
    ...
    entry_points = list(pybamm_reservoir_example.models).sort()
    models = Path("src/pybamm_reservoir_example/models/input/").glob("*.py")
...
def test_model_entry_point_load():
    ...
    # Loading parameter_sets through entry points
    model_instance = pybamm_reservoir_example.Model("SPM")
